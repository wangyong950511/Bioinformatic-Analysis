...
####多样本读取#####
# 读取CASH
parent_dir <- "~/R/scRNA/Data/NASH/ZWT/"
dir_name <- list.files(parent_dir)
dir_name
scRNAlist <- list()
for (i in 1:length(dir_name)) {
  counts <- Read10X(data.dir = paste("~/R/scRNA/Data/NASH/ZWT/", dir_name[i], sep = ""))
...
scRNAlist_merge$group <- ifelse(
  scRNAlist_merge$orig.ident %in% c("KO1", "KO2","KO3"),
  "Exp",
  "Ctrl"
)
...
# CD8T细胞 "Cd8a","Cd8b1","Gzmb","Prf1","Ifng"
# CD4T细胞 "Cd4","Cd3e","Ccr7","Cd44","Foxp3"



plots <- VlnPlot(
  scRNA_harmony,
  features = c("Cd3e","Cd4","Sell","Ccr7","Cd44"),
...
celltype[celltype$ClusterID %in% c(0,11), 2] = 'T cell'
celltype[celltype$ClusterID %in% c(4,5), 2] = 'Macrophages'
celltype[celltype$ClusterID %in% c(2), 2] = 'NK cell'
celltype[celltype$ClusterID %in% c(1), 2] = 'B cell'
celltype[celltype$ClusterID %in% c(6), 2] = 'Live cell'
celltype[celltype$ClusterID %in% c(3), 2] = 'Endothelial cell'
celltype[celltype$ClusterID %in% c(7), 2] = 'Monocyte'
celltype[celltype$ClusterID %in% c(10), 2] = 'Plasma cell'
celltype[celltype$ClusterID %in% c(8,9), 2] = 'DCs'
celltype[celltype$ClusterID %in% c(12), 2] = 'Mast Cells'
celltype
...
VlnPlot(scRNA_harmony, features = c("Zbtb17"), group.by = "celltype")
...
Idents(scRNA_harmony)="celltype"
Liver_cells <- subset(scRNA_harmony, idents = "T cell")
